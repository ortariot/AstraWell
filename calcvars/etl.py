...
def get_top_3_hotels_by_idea(hotel_recs: list, idea_name: str, idea_id) -> list[dict]:
    """
    """
    ...
    prices = list(map(lambda x: x.get('fields').get('price_per_day'), filtred_hotel_recs))
    stars = list(map(lambda x: x.get('fields').get('stars'), filtred_hotel_recs))
    ...
    hotel_rate_recs.sort(key=lambda x: x.get('fields', {}).get('rate'), reverse=True)

    return hotel_rate_recs[:3]


def get_top_3_flights_by_idea(flight_recs: list, idea_name: str, idea_id) -> list[dict]:
    """
    """
    ...
    prices = list(map(lambda x: x.get('fields').get('price'), filtred_flights_recs))
